...
if [ -z "$1" ]; then
    ...
    echo "./deploy_workflow.sh  <PROJECT_ID> <SERVICE-ACCOUNT-NAME@PROJECT-ID.iam.gserviceaccount.com> <REGION> <TIMEOUT_IN_SECONDS>"
    exit 1
fi
...
deploy_workflows_in_directory() {
    ...
    for file in "$dir"/*.yml "$dir"/*yaml; do
        ...
        if [ -f "$file" ]; then
            ...
            temp_file=$(replace_yaml_value "$temp_dir/$(basename "$file")" "timeout" $timeout)
