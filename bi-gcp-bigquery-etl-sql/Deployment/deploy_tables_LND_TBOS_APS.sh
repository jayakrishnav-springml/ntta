...
echo "Scripts to create Tables/schema for LND_TBOS_APS datasets"
...
if [ -z "$1" ]; then
    ...
    echo "./deploy_tables_LND_TBOS_APS.sh  <PROJECT_ID>"
    exit 1
fi

TYPE="LND_TBOS_APS"

if [[ $TYPE == "LND_TBOS_APS" ]] 
then
    for SQL_FILE in $(ls -R ../LND_TBOS_APS/*.sql)
    do 
        echo $SQL_FILE
