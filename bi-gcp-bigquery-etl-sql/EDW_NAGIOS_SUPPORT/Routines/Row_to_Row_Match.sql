CREATE OR REPLACE PROCEDURE `EDW_NAGIOS_SUPPORT.Row_to_Row_Match`(comparison_dataset_name STRING, dev_dataset_name STRING, table_name STRING, export_table STRING, primary_key STRING, excluded_columns STRING)
BEGIN
/*
#####################################################################################################
Proc Description: 
...
Example:   
--------------------------------------------------------------------------------------------------
DECLARE comparison_dataset_name STRING DEFAULT 'EDW_NAGIOS_APS';
DECLARE dev_dataset_name STRING DEFAULT 'EDW_NAGIOS';
DECLARE table_name STRING DEFAULT 'Host_Service_Event';
DECLARE export_table STRING DEFAULT 'Data_Validation.Host_Service_Event';
DECLARE primary_key STRING DEFAULT NULL;
DECLARE excluded_columns STRING DEFAULT 'edw_updatedate';
CALL `EDW_NAGIOS_SUPPORT.Row_to_Row_Match`(comparison_dataset_name, dev_dataset_name, table_name, export_table, primary_key, excluded_columns);
