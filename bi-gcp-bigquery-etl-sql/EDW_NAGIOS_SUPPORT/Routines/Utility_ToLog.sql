CREATE OR REPLACE PROCEDURE EDW_NAGIOS_SUPPORT.ToLog(logsource STRING, procstartdate DATETIME, logmessage STRING, logtype STRING, row_count INT64, query STRING)
BEGIN
/*
#####################################################################################################
Proc Description: This PROCEDURE uses Information Schema Jobs View to get the Info related 	to the jobs and loads the stats to EDW_NAGIOS_SUPPORT.ProcessLog Table for future references 
	Input:
...
*/
	...
	Then
		...
		IF EXISTS ( SELECT  1 FROM EDW_NAGIOS_SUPPORT.ProcessLog WHERE ProcessLog.logdate >= This_procstartdate AND ProcessLog.sessionid = Session_ID AND ProcessLog.queryid = query_id ) 
		THEN
    ...
    */
	...
	INSERT INTO EDW_NAGIOS_SUPPORT.ProcessLog -- Utility.ProcessLog
	(
