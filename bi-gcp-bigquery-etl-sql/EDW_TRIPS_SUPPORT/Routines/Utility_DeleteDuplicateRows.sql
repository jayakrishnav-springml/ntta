CREATE OR REPLACE PROCEDURE `EDW_TRIPS_SUPPORT.DeleteDuplicateRows`(table_name STRING, identifyingcolumns STRING, orderbystring STRING)
/*
USE EDW_TRIPS 
GO
...
*/
  ...
  BEGIN
    ...
    BEGIN
      ...
      CALL EDW_TRIPS_SUPPORT.ToLog(log_source, log_start_date, log_message, 'I', -1, -1);
      ...
      IF trace_flag = 1 THEN
        ...
        EXECUTE IMMEDIATE sql;
      END IF;
      CALL EDW_TRIPS_SUPPORT.ToLog(log_source, log_start_date, 'Completed Duplicate Rows cleanup', 'I', NULL, NULL);
    EXCEPTION WHEN ERROR THEN
