CREATE OR REPLACE PROCEDURE `LND_NAGIOS_SUPPORT.ToLog`(logsource STRING, procstartdate DATETIME, logmessage STRING, logtype STRING, row_count INT64, query STRING)
BEGIN
/*
#####################################################################################################
Proc Description: This PROCEDURE uses Information Schema Jobs View to get the Info related 	to the jobs and loads the stats to LND_NAGIOS_SUPPORT.ProcessLog Table for future references 
	Input:
...
Change Log:
--------------------------------------------------------------------------------------------
********    EGen        17-06-2024     New -- Taking REFERENCES from Utility.ToLog 
=============================================================================================
Example:   
---------------------------------------------------------------------------------------------
CALL LND_NAGIOS_SUPPORT.ToLog(log_source, log_start_date, log_message, 'I', -1, NULL);
CALL LND_NAGIOS_SUPPORT.ToLog(log_source, log_start_date, log_message, 'I', NULL, NULL);
...
*/
	...
	Then
		...
		IF EXISTS ( SELECT  1 FROM LND_NAGIOS_SUPPORT.ProcessLog WHERE ProcessLog.logdate >= This_procstartdate AND ProcessLog.sessionid = Session_ID AND ProcessLog.queryid = query_id ) 
		THEN
    ...
    */
	...
	INSERT INTO LND_NAGIOS_SUPPORT.ProcessLog -- Utility.ProcessLog
	(
