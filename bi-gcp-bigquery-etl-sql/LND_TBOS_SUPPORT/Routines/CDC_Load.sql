CREATE OR REPLACE PROCEDURE `LND_TBOS_SUPPORT.CDC_Load`(IN source_table STRING, IN target_table STRING, IN stage_table STRING, IN key_columns STRING, IN batch_end_date DATETIME, IN previous_run_id INT64, IN batch_name STRING, IN fullday_changedata_flag STRING, overlap_window_in_secs INT64)
BEGIN
/*
#####################################################################################################################################
Proc Description: 
...
Parameters : 
source_table - LND_TBOS_Qlik.<__ct tablename> 				:: ct table name from config table
target_table - LND_TBOS.<target_table> 					:: landing table name  from config table
stage_table  - LND_TBOS_STAGE_CDC.<stage_table>  		       :: cdc stagetable name from config table
...
batch_name - TRIPS/TRIPS_Finance_GL					:: Which group f table need to process for CDC
...
*/
    ...
    BEGIN
	 ...
	 DECLARE var_stage_insert_values_list STRING DEFAULT NULL;
       ...
       /*Inserting current run log record to cdc_batch_load table */      
        INSERT INTO LND_TBOS_SUPPORT.CDC_Batch_Load
           (      cdc_runid            ,
                  cdc_batch_name  ,
                  table_name      ,
                  batch_start_date,
		...
		THEN
         /*Fetching Minimum src_changedate when previous scucessful runs not exists  */      
        ...
        /*Fetching  batch_start_date when previous scucessful runs exists  */      
          SET batch_start_date_var=(
            SELECT
              ...
              WHERE
                table_name= target_table
                AND cdc_runid = (SELECT MAX(cdc_runid) from LND_TBOS_SUPPORT.CDC_Batch_Load where_st
   ...
   /*Setting time interval filter for current run*/
    ...
    END;
       ...
       /*generating update clause and select column and valuse list for dynamic merge*/
	...
	(
		   ...
		   FROM
				  LND_TBOS_SUPPORT.CDC_Full_Load_Config a
		   WHERE
	...
	SET merge_query = "  MERGE INTO " ||target_table ||" T USING    (SELECT *  FROM   " || stage_table ||") S  ON   " ||on_clause ||"   WHEN MATCHED THEN UPDATE SET " ||update_clause ||" WHEN NOT MATCHED THEN  INSERT  " ||insert_columns_list ||"  VALUES   (" ||insert_values_list ||"); ";
       ...
       /*Metrics Realetd Block starts here*/
