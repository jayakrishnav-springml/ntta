CREATE OR REPLACE PROCEDURE `LND_TBOS_SUPPORT.ToLog`(logsource STRING, procstartdate DATETIME, logmessage STRING, logtype STRING, row_count INT64, query STRING)
BEGIN
/*
#####################################################################################################
Proc Description: This PROCEDURE uses Information Schema Jobs View to get the Info related 	to the jobs and loads the stats to LND_TBOS_SUPPORT.ProcessLog Table for future references 
	Input:
...
Example:   
---------------------------------------------------------------------------------------------
CALL LND_TBOS_SUPPORT.ToLog(log_source, log_start_date, log_message, 'I', -1, NULL);
...
*/
	...
	Then
		...
		IF EXISTS ( SELECT  1 FROM LND_TBOS_SUPPORT.ProcessLog WHERE ProcessLog.logdate >= This_procstartdate AND ProcessLog.sessionid = Session_ID AND ProcessLog.queryid = query_id ) 
		THEN
    ...
    */
	...
	INSERT INTO LND_TBOS_SUPPORT.ProcessLog -- Utility.ProcessLog
	(
